...
```{r}
acs_ca_c <- getCensus(name = "acs/acs5",
                    vintage = 2020,
...
for (i in 1:dim(acs_ca_c)[1]) {
  acs_ca_c$location[i] <- gsub(" County","",acs_ca_c$location[i])
  acs_ca_c$location[i] <- tolower(acs_ca_c$location[i])
}
# Join the ACS data with the map data.
acs_ca_map <- inner_join(acs_ca_c,ca_map,by = join_by(location == subregion))
```

After you do this, plot a map of Illinois with Counties colored by per capita income.
```{r,fig.height = 6.5, fig.width = 6}
ggplot(acs_ca_map) + geom_polygon(aes(x = long, y = lat, group = group, fill = income))
```
...
```{r}
cleaned_acs_ca_c <- acs_ca_c %>%
  select(location, pop, hh_income, income) %>%
  na.omit()
```
...
dist_mtx_acs_ca_c <- dist(cleaned_acs_ca_c[, -1])  # excluding the county identifier
# Perform hierarchical clustering
hc_c <- hclust(dist_mtx_acs_ca_c, method = "ward.D2")
```
...
# Plot the dendrogram
plot(hc_c)
rect.hclust(hc_c, k = 5)  # Here 'k' is the number of clusters you choose
# Cut the dendrogram to form clusters
clusters <- cutree(hc_c, k = 5)
# Merge the cluster memberships with the original data
cleaned_acs_ca_c$cluster <- clusters
```
...
```{r}
acs_map <- inner_join(cleaned_acs_ca_c,ca_map,by = join_by(location == subregion))
...
```{r}
acs_ca_t <- getCensus(name = "acs/acs5",
                    vintage = 2020,
                    vars = c("NAME", "B01003_001E", "B19013_001E", "B19301_001E"),
                    region = "tract:*",
                    regionin = "state:06",
                    key = cs_key) %>% rename(pop = B01003_001E,
                   hh_income = B19013_001E,
                   income = B19301_001E)
acs_ca_t[acs_ca_t == -666666666] <- NA
head(acs_ca_t)
```
```{r}
# Clean NAME in acs_ca_t
acs_ca_t %<>%
  separate(NAME, c("ct","location","state_name"), sep = ",") 
for (i in 1:dim(acs_ca_t)[1]) {
  acs_ca_t$location[i] <- gsub(" County","",acs_ca_t$location[i])
  acs_ca_t$location[i] <- tolower(acs_ca_t$location[i])
}
acs_ca_t$ct <- trimws(acs_ca_t$ct)
acs_ca_t$location <- trimws(acs_ca_t$location)
acs_ca_t$state_name <- trimws(acs_ca_t$state_name)
```
...
wcss <- map_dbl(1:20, function(k) {
  kmeans(cleaned_acs_ca_t[,-1], centers = k, nstart = 20)$tot.withinss
})
...
k <- 3
# Create the distance matrix
dist_mtx_acs_ca_t <- dist(cleaned_acs_ca_t[, -1])  # excluding the county identifier
# Perform hierarchical clustering
hc_t <- hclust(dist_mtx_acs_ca_t, method = "ward.D2")
# Cut the dendrogram to form clusters
clusters <- cutree(hc_t, k = k)
# Merge the cluster memberships with the original data
cleaned_acs_ca_t$cluster <- clusters
```

```{r}
cleaned_acs_ca_t %>% group_by(cluster) %>% summarise(m_pop = mean(pop),m_hh=mean(hh_income),m_inc=mean(income))
```
```{r}
cleaned_acs_ca_t %>% group_by(cluster) %>% count(location) %>% top_n(1, wt = n) 
...
optimize_kmeans <- function(k){
  scaled_data <- scale(cleaned_acs_ca_t[, 2:4]) 
  set.seed(123)  # Set seed for reproducibility
  kmeans_result <- kmeans(scaled_data, centers = k, nstart = 20)$tot.withinss
  cluster_var_name <- paste("cluster_k", k, sep = "")
  cleaned_acs_ca_t[[cluster_var_name]] <- kmeans_result
  return(cleaned_acs_ca_t)
}

cleaned_acs_ca_t <- optimize_kmeans(2)
```
...
for (k in k_values) {
  cleaned_acs_ca_t <- optimize_kmeans(k)
}
```
